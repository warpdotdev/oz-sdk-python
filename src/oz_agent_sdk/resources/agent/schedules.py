...
from ...types.agent import schedule_create_params
from ...types.agent import schedule_update_params
...
class SchedulesResource(SyncAPIResource):
    ...
    *,
    ...
    timeout: float | httpx.Timeout | None | NotGiven = not_given,) -> ScheduledAgentItem:
    ...
    *,
    ...
    timeout: float | httpx.Timeout | None | NotGiven = not_given,) -> ScheduledAgentItem:
        ...
        if not schedule_id:
          raise ValueError(
            f'Expected a non-empty value for `schedule_id` but received {schedule_id!r}'
          )
        return self._get(
